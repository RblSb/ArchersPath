...
class UpdateAIControl implements ISystem {
	...
	function update(ai:AI, control:Control, body:Body, pos:Position, size:Size, speed:Speed, coll:Collision):Void {
		...
		if (coll.down) { // on ground
			...
			} else if (!canJumpOver(pos.x + side * tileSize, pos.y)) {
		...
		if (pos.x + size.w + speed.x > lvl.w * tileSize - 1) setMoveDirection(keys, 1);
	...
	inline function block(x:Float, y:Float):Bool {
		var ix = Std.int(x / tileSize);
		var iy = Std.int(y / tileSize);
		return lvl.getTile(1, ix, iy).props.collide;
	}

	inline function canFallDown(x:Float, y:Float, n:Int):Bool {
		var safe = false;
		for (i in 1...1 + n) {
			if (block(x, y + tileSize * i)) {
				safe = true;
				break;
			};
		}
