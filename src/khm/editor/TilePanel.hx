...
class TilePanel {
	...
	function setTile(p:Pointer):Void {
		var tx = Std.int((p.x - x) / tileSize);
		var ty = Std.int((p.y - y) / tileSize);
		var cord = ty * w + tx;
	...
	function drawBg(g:Graphics, x:Int, y:Int, w:Int, h:Int):Void {
		g.color = BG_COLOR;
		g.fillRect(x - 1, y, w * tileSize + 1, h * tileSize);
	}
	...
	function drawSelection(g:Graphics, x:Int, y:Int, w:Int, h:Int):Void {
		var offX = editor.tile == 0 ? 0 : current * tileSize;
		var ix = offX % (w * tileSize);
		var iy = Std.int(offX / (w * tileSize)) * tileSize;
