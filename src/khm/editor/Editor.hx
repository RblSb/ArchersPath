...
class Editor extends Screen {
	...
	function initButtons():Void {
		...
		buttons = [
			...
			new Button({x: Screen.w - h - tilePanel.w * tileSize, y: 0, img: i.icons_play, keys: [KeyCode.Zero]})
		];
	...
	function updateCamera():Void {
		var w = Screen.w;
		var h = Screen.h;
		var pw = tilemap.w * tileSize;
		var ph = tilemap.h * tileSize;
		var camera = tilemap.camera;
		var offset = BTN_SIZE;
		var maxW = w - pw - offset - tilePanel.w * tileSize;
	...
	override function onRender(frame:Canvas):Void {
		...
		g.drawRect(tilemap.camera.x, tilemap.camera.y - 1,
			tilemap.w * tileSize + 1,
			tilemap.h * tileSize + 1
		);
	...
	function drawGrid(g:Graphics):Void {
		...
		for (ix in 0...tilemap.w) {
			for (iy in 0...tilemap.h) {
				g.drawRect(
					tilemap.camera.x + ix * tileSize + 1,
					tilemap.camera.y + iy * tileSize,
	...
	function drawCursor(g:Graphics):Void {
		if (tool == hand) return;
		g.color = 0x88000000;
		var px = x * tileSize + tilemap.camera.x;
		var py = y * tileSize + tilemap.camera.y - 1;
		g.drawRect(px, py, tileSize + 1, tileSize + 1);
		if (tool == arrow) return;

		if (tile == 0) {
			g.color = 0x88FF0000;
			g.drawLine(px, py + 1, px + tileSize, py + tileSize + 1);
			g.drawLine(px + tileSize, py + 1, px, py + tileSize + 1);
		}
		if (tile < 1) return;
		g.color = 0xFFFFFFFF;
		drawTile(
	...
	public function drawTile(g:Graphics, layer:Int, x:Float, y:Float, id:Int):Void {
		...
		var tx = (id % tileset.w) * tileSize;
		var ty = Std.int(id / tileset.w) * tileSize;
		g.drawSubImage(
			tileset.img, x, y,
